...
from ratplan_planner.primitives.features import FEATURE_NAMES, N_FEATURES, features_for_position, features_for_positions
...
def test_position_features_match_working_crs_offset_and_elevation(flat_dtm):
    ...
    x, y, elevation, sin_b, cos_b, _los = features.values[0]
...
def test_bearing_defaults_at_zero_distance(flat_dtm):
    ...
    assert features.values[0][3] == pytest.approx(0.0)
    assert features.values[0][4] == pytest.approx(1.0)
...
def test_bearing_periodic_embedding_keeps_near_north_points_close(flat_dtm):
    ...
    nne, nnw, south = features.values[0, 3:5], features.values[1, 3:5], features.values[2, 3:5]
