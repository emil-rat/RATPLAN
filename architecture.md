...
2026-09-23 - Claude (session with Emil J) — the connectivity module was pulled out of `ratplan_planner/` into its own top-level `connectivity_module/` folder: `ITWOM/` moved wholesale under it (`connectivity_module/ITWOM/...`), and the concrete `Scan`/GP implementation (`ratplan_planner`'s former `models/coverage.py`, `primitives/{scan,gp_correction,kernel,features}.py`) moved alongside it as a new `ratplan_connectivity` package, so `ratplan_planner` now holds only the DP/mission-planning code. Updated every current `ITWOM/...` path reference below (§§3–4, §7–9) accordingly; §§5–6's already-non-authoritative package-layout diagram was left untouched since it's historical reference only.
...
| Use [itmlogic](https://github.com/edwardoughton/itmlogic) (Python port of ITM only) | MIT (+ must retain NTIA's attribution text, per ITM's own terms) | **Implemented** as [`connectivity_module/ITWOM/itm/`](./connectivity_module/ITWOM/itm/), as-is. If the rest of RATPLAN's stack is Python, this avoids a C++ binding layer. Not yet cross-validated against the NTIA C++ reference — see `OPEN_ISSUES.md`. |
| SPLAT! (unmodified upstream install, e.g. [jmcmellen/splat](https://github.com/jmcmellen/splat)), for ITWOM specifically | GPL-2.0 (package-level) | **Implemented** as [`connectivity_module/ITWOM/itwom/`](./connectivity_module/ITWOM/itwom/) (the GPL-2.0 Docker service, unmodified upstream SPLAT! pinned to a specific commit) + [`connectivity_module/ITWOM/itwom_client/`](./connectivity_module/ITWOM/itwom_client/) (RATPLAN's own arm's-length caller). Verified end-to-end against a synthetic obstructed path; ITWOM vs. classic ITM (`-olditm`) diverge ~14 dB as expected. Terrain is synthetic, not real DEM data yet — see `OPEN_ISSUES.md`. |
...
- ~~Deliberately **not** wired to `connectivity_module/ITWOM/itm/` or any `Scan` implementation yet — once `rattfallan/primitives/scan.py` exists and calls into `connectivity_module/ITWOM/itm/`'s real service boundary (§3.1), `mapviz` is where a coverage-heatmap layer gets added back, reusing this same hillshade/roads base rather than starting a new map tool.~~ — superseded 2026-09-25: that layer was built instead as a separate `RATPLAN/simulation_tool/` (own README), per Emil's explicit request, not folded back into `mapviz`. `mapviz` stays scoped to terrain+roads sanity-checking as described above; `simulation_tool/` is where relay placement (`ItmGridScan`) and observation-driven GP correction (`GpCorrector`) actually get exercised visually, reusing this same hillshade/roads base plus `rattfallan-demo`'s coverage-heatmap rendering conventions (`margin_db` color ramp).

---

## 9. Open items for next session

- ~~Confirm itmlogic supports point-to-point mode~~ — done. Implemented as [`connectivity_module/ITWOM/itm/`](./connectivity_module/ITWOM/itm/) (self-contained module, own `pyproject.toml`/`.venv`, itmlogic as a normal dependency, wrapped rather than forked). See [`connectivity_module/ITWOM/itm/README.md`](./connectivity_module/ITWOM/itm/README.md) for usage/scope and [`OPEN_ISSUES.md`](./OPEN_ISSUES.md) for what's still unresolved about it (itmlogic issue #64, uncalidated-against-NTIA gap, uncalibrated ground constants).
- ~~Build the ITWOM service~~ — done. Implemented as [`connectivity_module/ITWOM/itwom/`](./connectivity_module/ITWOM/itwom/) (GPL-2.0 Docker service, unmodified upstream SPLAT! pinned to commit `08f06f5`) + [`connectivity_module/ITWOM/itwom_client/`](./connectivity_module/ITWOM/itwom_client/) (RATPLAN's own arm's-length client). See both READMEs and `OPEN_ISSUES.md`'s "ITWOM/itwom service" section for what's still open (synthetic terrain, single-tile limit, no independent reference validation).
- No performance budget set yet for `Scan()`'s grid-batched ITM calls — needed once relay-placement iteration counts (pseudocode.md step 1) are estimated. `simulation_tool/server.py` picks a coarser interactive grid (`SIMULATION_TOOL_CELL_SIZE_M`/`SIMULATION_TOOL_MAX_EXTENT_M` env vars, default 50m/800m vs. `ItmGridScan`'s own 25m/1500m library defaults) as a heuristic stopgap to keep map-click placement responsive — not a derived budget, still open.
- `rattfallan/` package (§6) doesn't exist yet this restart — needs building once `connectivity_module/ITWOM/itm/`'s `Scan` contract (§3.1) is stable enough for `primitives/scan.py` to call into it. The GP/kriging correction layer (`model.md` §3) is now implemented as `connectivity_module/ratplan_connectivity/primitives/gp_correction.py` (`ratplan_planner`'s eventual `primitives/scan.py` will need to decide how it wires into that, once it exists).
- ~~`mapviz/` (§8) ships with no coverage layer by design (scope decision this session) — revisit once there's a `Scan` implementation to visualize.~~ — superseded 2026-09-25: see §8's note above. `simulation_tool/` is that visualization, built as its own tool rather than an addition to `mapviz/`.
